...
AD9833 Ad9833(FNC_PIN);  // Defaults to 25MHz internal reference frequency
...
void /*long*/ readAnalogAndSetFreqInSetup() {
  ...
  for (int j = 1; j <= iterations; j++) {
    ...
    Ad9833.SetFrequency(REG0, (float)freq);
    delay(20);
  }
  ...
  Ad9833.SetFrequency(REG0, (float)ifreq);
...
void readAnalogAndSetFreqInLoop() {
  ...
  if (curr - prevReadAnalogTime > 1000 * 5) { //выбор времени изменения частоты.1-10 сек.
    ...
    Ad9833.SetFrequency(REG0, (float)minimalFreq);
    delay(20);
...
void setup() {
  ...
  Ad9833.ApplySignal(SINE_WAVE, REG0,(float) FREQ_MIN);
